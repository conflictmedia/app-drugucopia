...
test.describe("Per-Substance Tolerance Notifications E2E", () => {
  ...
  test("global tolerance notification toggles are present with correct initial states", async ({
    page,
  }) => {
    // All three global toggle sections should be present
    await expect(
      page.locator('label:has-text("High / Very High")').first(),
    ).toBeVisible();
    await expect(
      page.locator('label:has-text("Low / Moderate")').first(),
    ).toBeVisible();
    await expect(
      page.locator('label:has-text("Baseline recovered")').first(),
    ).toBeVisible();

    // Each should have a button showing On/Off state
    await expect(
      page.locator('label:has-text("High / Very High") >> button').first(),
    ).toBeVisible();
    await expect(
      page.locator('label:has-text("Low / Moderate") >> button').first(),
    ).toBeVisible();
    await expect(
      page.locator('label:has-text("Baseline recovered") >> button').first(),
    ).toBeVisible();
  ...
  test("Substance Selection section is present", async ({ page }) => {
    const substanceSelectionButton = page.locator(
      'button:has-text("Substance Selection")',
    );
    await expect(substanceSelectionButton).toBeVisible();
    await expect(substanceSelectionButton).toContainText("Substance Selection");
  });

  test("Substance Selection can be expanded to show search", async ({
    page,
  }) => {
    const substanceSelectionButton = page.locator(
      'button:has-text("Substance Selection")',
    ...
    const searchInput = page
      .locator('input[placeholder="Search substances..."]')
      .first();
  ...
  test("Test Check Now button is present", async ({ page }) => {
    await expect(
      page.locator('button:has-text("Test Check Now")'),
    ).toBeVisible();
  });
});

test.describe("Cross-tab sync for tolerance notifications", () => {
  test("settings are loaded from localStorage on page load", async ({
    page,
    context,
  }) => {
    ...
    const highToggle = page
      .locator('label:has-text("High / Very High") >> button')
      .first();
    ...
    await expect(
      page2.locator('label:has-text("High / Very High") >> button').first(),
    ).toHaveText("On");
