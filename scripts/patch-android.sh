...
#
# The drawable referenced below (ic_splash) MUST exist in res/drawable/ or the
# Gradle `processUniversalReleaseResources` task fails with
# "resource drawable/ic_splash not found". We ensure it exists in step 3d below.
THEMES_FILE="$STYLES_DIR/themes.xml"
# Always overwrite — Tauri's own themes.xml may have been regenerated by
# `tauri icon` between runs and would otherwise lack our splash entries.
cat > "$THEMES_FILE" <<'EOF'
<?xml version="1.0" encoding="utf-8"?>
<resources>
...
EOF
  ok "Wrote themes.xml with Theme.Drugucopia.Splash (Android 12+)"

# ─── 3d. Ensure the splash drawable exists ───────────────────────────────────
# themes.xml references @drawable/ic_splash; if it is missing, the Android
# resource linker aborts the build. Same logic as scripts/android.sh — kept
# inline here so patch-android.sh is self-contained when run on its own.
...
if [ -z "$SPLASH_SRC" ]; then
  echo "Error: No source PNG found for splash drawable — themes.xml references @drawable/ic_splash and the Android build will fail"
  exit 1
fi
...
PYEOF
  ...
  if [ "$SPLASH_CREATED" != "1" ]; then
    ...
    cp "$SPLASH_SRC" "$DRAWABLE_DIR/ic_splash.png" || { echo "Error: Failed to copy $SPLASH_SRC to $DRAWABLE_DIR/ic_splash.png"; exit 1; }
    SPLASH_CREATED=1
  fi
fi

if [ ! -f "$DRAWABLE_DIR/ic_splash.png" ]; then
  echo "Error: Splash drawable is still missing at $DRAWABLE_DIR/ic_splash.png — themes.xml references @drawable/ic_splash, the Android build cannot succeed"
  exit 1
fi
